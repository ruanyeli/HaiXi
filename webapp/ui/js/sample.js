...
//    fbx.workbook(spread);
    fbx1 = new GC.Spread.Sheets.FormulaTextBox.FormulaTextBox(document.getElementById('formulaBar'), {rangeSelectMode: true});
    fbx1.workbook(spread);
...
function processSpreadContextMenu(e) {
    ...
    if (isHideContextMenu) {
        ...
        $contextMenu.css({left: e.pageX + "px", top: e.pageY - document.getElementById("ss").clientHeight + "px"});
        $contextMenu.show();
			...
			for(var n=selectionCells0[i].col;n<selectionCells0[i].col+selectionCells0[i].colCount;n++){
				if(n>=1){
					if(i>=1){
						...
						if(JSON.stringify(xArray2)!=JSON.stringify(xArray3)){
							//alert(x2);
							//alert(sheet0.getArray(2,n,1,selectionCells0[i].colCount));
							xArray2=xArray2.concat(sheet0.getArray(2,n,1,selectionCells0[i].colCount));
						}						
					}else{
						var array0;
						for(var k=n;k>=1;k--){
							array0=sheet0.getValue(1,k);
							if(array0!=null){
								array0=array0.split("/");
								index_j=array0[array0.length-1];
								break;
							}						
						}						
						xArray2=sheet0.getArray(2,n,1,selectionCells0[i].colCount);						
					}
					break;
				}				
			}			
...
//        	alert(left[i]);
        ...
        for(var j=0;j<xArray2[0].length;j++){
        	upper[j]=index_j+'('+xArray2[0][j]+')';
        }
...
function createOptions(matrix, upper_arg, left_arg, input_title, type) {// 生成绘图数据,type=1折线和柱状2折柱混合3饼状图，饼状的先不管了
	...
	if (existChartNum == 0) { 
		...
		+"<div style='height:40px'><a href='#' onclick='delSearch(this)' class='easyui-linkbutton' iconCls='icon-remove'>删除</a></div>" +
				"</td></tr>");
            ...
            for(var i = 0; i < selections.length; i++){
                ...
                for(var row = selection.row; row < selection.row + selection.rowCount; row++){
                    for(var col = selection.col; col < selection.col + selection.colCount; col++){
                        ...
                        if(value !== null && value !== undefined && value !== ""){//当单元格值有效时
                            if(isFinite(value)){
                                numCount++;
...
function caculate(){	
	var activeSheet=spread.getActiveSheet();   
    var str=fbx1.text();//得到位置
	console.log("str is "+str);
	var txt=str.split("!")[1];//抛去
	var val=txt.split(":");
    ...
    activeSheet.setValue(row,col,'2014');
...
function mediannum(number,count){//中位数
	number.sort(sequence);
	console.log("排序后的number如下：");
	console.log(number);
	...
	console.log("中位数是 " + number[result]);
	return number[result];
}

function max(number,count){
	number.sort();//number.sort()对number数组进行排序		
	console.log("最大值是："+ number[count-1]);
	return number[count-1];
}

function min(number,count){
	number.sort();
	console.log("最小值是:" +number[0]);
	return number[0];
}

function variance(number,count,mean){//计算方差
	console.log("平均数是："+mean);
	var sum=0;
