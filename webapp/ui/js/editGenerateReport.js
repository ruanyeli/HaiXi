...
var deid=parent.$("#myDialog").dialog("options").data.deid; 
var dedepart=0;
...
//				console.log(JSON.stringify(resp.data));
				if(resp.resultCode=='0'){
					$("#dedepartment").combobox({
						valueField:"depid",
						textField:"depname",
						value:parent.$("#myDialog").dialog("options").data.dedepartment,
						data:resp.data
					});
				}else{
	...
	$.ajax({
		type:"post",
		url:"/derive/getDestatement",
		data:{
			"deid":deid
		},
		success:function(resp){
			if(resp.resultCode=='0'){
				var destatement=resp.data;
				$("#dename").val(destatement.dename);
				if(_depid==0){
					$("#dedepartment").combobox('setValue',destatement.dedepartment);
				}
			}else{
				layer.msg(resp.resultDesc);
			}
	...
	$.ajax({
		type:"post",
		url:"/derive/getDestatementContent",
		async:false,
		data:{
			"deid":deid
		},
		success:function(res){
			if(res.resultCode=="0"){
				...
				dehead=res.data.deheader;
				deformula=res.data.deformula;
				sheet=spread.getActiveSheet();
				sheet.suspendPaint(); 
				for(var i=0;i<dehead.length;i++){//得到上表头
					var headname=dehead[i].deheadname;
					var headlocation=dehead[i].deheadlocation;
					var hdrow=parseInt(headlocation.split("@#")[0]);
					var hdcol=parseInt(headlocation.split("@#")[1]);
					var hdrowspan=parseInt(dehead[i].deheadrowspan);
					var hdcolspan=parseInt(dehead[i].deheadcolspan);			
					sheet.addSpan(hdrow, hdcol, hdrowspan, hdcolspan);
					sheet.setValue(hdrow,hdcol,headname);
				}
				for(var i=0;i<deformula.length;i++){//得到上表头
					var facontent=deformula[i].facontent;
					var falocation=deformula[i].falocation;
					var hdrow=parseInt(falocation.split("@#")[0]);
					var hdcol=parseInt(falocation.split("@#")[1]);
					sheet.setValue(hdrow,hdcol,facontent);
				}
				sheet.resumePaint();
			}else{
	...
	$("#saveEdit").bind('click', function(){//左表头传值
		var detype;
		if(leftRow==0)//没有左表头
			detype=0;
		else detype=1;
		...
		if(_depid==0){
			if($("#dedepartment").combobox('getValue')==''||$("#dedepartment").combobox('getValue')==null){
				layer.msg("请选择部门");
				return;
			}
			_depid=$("#dedepartment").combobox('getValue');
		}
	$.ajax({
		type : "post",
		url :"/derive/editDestatement",
		data : {
			"deid":deid,
